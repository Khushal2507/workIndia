...
async function addTrains(
  ...
  try {
    const train = await db.query(
      "INSERT INTO trains(train_name, source_station, destination, seat_capacity, arrival_time_at_source, arrival_time_at_destination) VALUES (?, ?, ?,?,?,?)",
      [
        train_name,
        source,
        destination,
    ...
    const getTrain = await db.query("SELECT * FROM trains where id = ?", [
      train[0].insertId,
    ]);
    return getTrain[0][0];
  } catch (err) {
...
async function getTrains(source, destination) {
  const trains = await db.query(
    "SELECT * FROM trains where source_station = ? and destination = ?",
    [source, destination]
  );
  return trains[0];
}
...
module.exports.addTrains = addTrains;
